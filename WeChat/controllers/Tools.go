...
// 开启或者关闭账号服务
func (c *ToolsController) SwitchAccountService() {
	var ParamData Tools.SetEnableServiceParam
	...
	WXDATA := Tools.EnableAccountService(ParamData)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}
...
// @Summary CDN下载高清图片
// @Param	body		body 	Tools.CdnDownloadImageParam true
// @Success 200
...
// @Success 200
// @router /DownloadVoice [post]
func (c *ToolsController) DownloadVoicePlus() {
	var ParamData Tools.DownloadVoiceParam
	data := c.Ctx.Input.RequestBody
	err := json.Unmarshal(data, &ParamData)
	if err != nil {
	...
	WXDATA := Tools.DownloadVoicePlus(ParamData)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}
...
// 获取账号统计信息
func (c *ToolsController) GetAccountInfo() {
	wxid := c.GetString("wxid")
	WXDATA := Tools.GetAccountInfo(wxid)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}

// @Summary 文件上传
// @Param	body		body 	Tools.UploadParam    true	"文件上传"
// @Success 200
// @router /UploadFile [post]
func (c *ToolsController) UploadFile() {
	var ParamData Tools.UploadParam
	data := c.Ctx.Input.RequestBody
	err := json.Unmarshal(data, &ParamData)
	if err != nil {
		Result := models.ResponseResult{
	...
	WXDATA := Tools.UploadAppAttach(ParamData)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}
