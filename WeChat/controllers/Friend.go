...
// 检测单线好友
func (c *FriendController) CheckSingleFriend() {
	wxid := c.GetString("wxid")
	WXDATA := Friend.CheckSingleFriend(wxid)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}
...
// @Tags		添加/移除黑名单
// @Summary	批量删除好友
// @Param		body	body	Friend.BatchDelFriendParam	true	" "
// @Failure	200
// @router		/Friend/BatchDelFriend [post]
func (c *FriendController) BatchDelFriend() {
	var Data Friend.BatchDelFriendParam
	err := json.Unmarshal(c.Ctx.Input.RequestBody, &Data)
	if err != nil {
		Result := models.ResponseResult{
			Code:    -8,
	...
	WXDATA := Friend.BatchDelFriend(Data)
	c.Data["json"] = &WXDATA
	c.ServeJSON()
}
