...
// @Summary 发送分享链接消息
// @Param	body		body 	Msg.SendShareLinkMsgParam   true	"Title==标题 Desc==描述 Url==跳转地址 ThumbUrl==图片展示"
// @Success 200
// @router /ShareLink [post]
func (c *MsgController) ShareLink() {
	var ParamData Msg.SendShareLinkMsgParam
	data := c.Ctx.Input.RequestBody
	err := json.Unmarshal(data, &ParamData)
	if err != nil {
		Result := models.ResponseResult{
	...
	WXDATA := Msg.SendAppMsg(Msg.SendAppMsgParam{
		Wxid:   ParamData.Wxid,
		ToWxid: ParamData.ToWxid,
		Xml:    "<appmsg appid=\"\"  sdkver=\"1\"><title>" + ParamData.Title + "</title><des>" + ParamData.Desc + "</des><action></action><type>5</type><showtype>0</showtype><soundtype>0</soundtype><mediatagname></mediatagname><messageext></messageext><messageaction></messageaction><content></content><contentattr>0</contentattr><url>" + ParamData.Url + "</url><lowurl></lowurl><dataurl></dataurl><lowdataurl></lowdataurl><songalbumurl></songalbumurl><songlyric></songlyric><appattach><totallen>0</totallen><attachid></attachid><emoticonmd5></emoticonmd5><fileext></fileext><cdnthumbaeskey></cdnthumbaeskey><aeskey></aeskey></appattach><extinfo></extinfo><sourceusername></sourceusername><sourcedisplayname></sourcedisplayname><thumburl>" + ParamData.ThumbUrl + "</thumburl><md5></md5><statextstr></statextstr><directshare>0</directshare></appmsg><fromusername></fromusername>",
		Type:   5,
	})
	c.Data["json"] = &WXDATA
	c.ServeJSON()
